...
export function AppRouter() {
  ...
  const handleAutoCreateGame = async (selectedCase?: LegalCase, gameSettings?: any, selectedRole?: string) => {
    ...
    try {
      ...
      try {
        ...
        addNotification({ 
          type: 'error', 
          message: 'Unable to create game. Please try again or contact support.' 
        })
        
        // Instead of going to lobby (duplicate settings), go back to case selection
        setCurrentPage('case-preview')
      }
    }
  }
  ...
  return (
    <>
      ...
      {currentPage === 'role-selection' && selectedCaseForQuickGame && (
